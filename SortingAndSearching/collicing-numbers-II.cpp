...
int solve(vector<int> &values, vector<int> &indices, int n, int a, int b,
          int &ans) {
  set<pair<int, int>> updated_pairs;

  if (values[a] < n)
    updated_pairs.insert({values[a], values[a] + 1});
  if (values[b] < n)
    updated_pairs.insert({values[b], values[b] + 1});
  if (values[a] > 1)
    updated_pairs.insert({values[a] - 1, values[a]});
  if (values[b] > 1)
    updated_pairs.insert({values[b] - 1, values[b]});
